...
def get_url(page):
    # get_house_id()
    a_list = []
    url = 'https://lz.lianjia.com/ershoufang/pg' + str(page)
    parse_page(get_house_id(url), a_list)
...
def sprider():
    pool = Pool()
    url_list = pool.map(get_url, [i for i in range(1, 101)])
    # url_list = get_url()
    details_list = []
    # details_list += parse_details(get_details(url_list[0]))
    details_list += pool.map(parse_details, url_list)
    # for url in url_list:
    #     # get_details(url)
